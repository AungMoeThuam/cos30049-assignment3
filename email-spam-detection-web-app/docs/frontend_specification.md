...
#### 5.1.3. Feature Radar / Spider Chart (D3.js)

- **Purpose**: Visualize the 9 extracted numeric features of the submitted email compared against spam and ham training averages, helping users understand _why_ the email was classified as spam or ham.
- **Library**: D3.js (via `useRef` + `useEffect` pattern — D3 owns the SVG DOM directly)
- **Component**: `src/components/charts/FeatureRadarChart.jsx`
- **Axes (9 total)**: `num_urls`, `num_exclamation`, `num_question`, `num_dollar`, `num_all_caps`, `num_numbers`, `word_count`, `capital_ratio`, `emoji_count`
- **Three Overlapping Polygons**:
  - 🔵 **This Email** — live features from the `/predict` response
  - 🔴 **Spam Average** — pre-computed training averages from `GET /feature-averages`
  - 🟢 **Ham Average** — pre-computed training averages from `GET /feature-averages`
- **Normalization**: Each axis is scaled 0–1 relative to the per-feature dataset maximum so all 9 axes are visually comparable regardless of unit magnitude.
- **Interactivity**:
  - Hover on any polygon vertex → tooltip showing the raw value for that feature
  - Animated polygon draw on mount via `d3.transition().duration(800)`
  - Clickable legend to toggle individual polygon visibility
- **Data Sources**:
  - `singleAnalysisResult.features` — returned by `POST /predict` response
  - `featureAverages.spam` / `featureAverages.ham` — fetched once on page load from `GET /feature-averages`

#### 5.1.4. Sentence-Level Interactive Heatmap
...
- Each card includes the model name and a linear progress bar indicating the spam (red) vs. ham (green) ratio.
  - _Example (Naive Bayes Card)_: Spam: 65 (43.3%) | Ham: 85 (56.7%).
...
async function analyzeCsvBatch(file, topN = 10) {
  ...
  const response = await fetch(
    `http://localhost:8000/api/v1/predict/csv?top_n=${topN}`,
    {
      method: "POST",
      body: formData,
    },
...
### 6.3. Fetch Feature Averages

- **Endpoint**: `GET http://localhost:8000/api/v1/feature-averages`
- **Called**: Once on page load via `useEffect` in `SpamCheckPage.jsx`, result stored in `featureAverages` state.
- **Javascript Fetch Implementation**:

```javascript
async function fetchFeatureAverages() {
  const response = await fetch("http://localhost:8000/api/v1/feature-averages");
  return await response.json();
  // Returns: { spam: { num_urls, num_exclamation, ... }, ham: { ... } }
}
```
...
- **`singleAnalysisResult`**: JSON response from `/predict` (or `null`). Now includes a `features` key containing the 9 extracted numeric features for the submitted email.
...
- **`featureAverages`**: `{ spam: { num_urls, ... }, ham: { num_urls, ... } }` fetched once on page load from `GET /feature-averages`. Used to render the spam/ham reference polygons in the Feature Radar Chart.
