...
1. Paste raw email text or upload single email files (`.eml`, `.msg`, `.txt`) to analyze sentence-by-sentence.
...
### 3.1. Main Input Card (`<Card sx={{ p: 3, mb: 4, borderRadius: 2, boxShadow: 3 }}>`)
...
## 4. Dashboards (Conditional Rendering)

Depending on the tab used to submit the request, the page displays one of two dashboard layouts:

### 4.1. Dashboard A: Single Email Dashboard (Used for Tab 1 & Tab 2)

#### 4.1.1. Model Selection Card
...
#### 4.1.2. Active Model Overall Result Card
...
#### 4.1.3. Sentence-Level Interactive Heatmap
...
### 4.2. Dashboard B: CSV Batch Summary Dashboard (Used for Tab 3)

#### 4.2.1. Processed Count Banner
* A large card stating the total number of processed email rows (e.g. `"Total Emails Processed: 150"`).

#### 4.2.2. Multi-Model Distribution Grid
...
#### 4.2.3. Top Spam Keywords Visualizer
...
### 5.1. Submit Single Email (Text / Single File)
